...
model_urls = {
    'hrnetocr': 'http://sceneparsing.csail.mit.edu/model/pretrained_resnet/hrnetv2_w48-imagenet.pth',
}
...
class HRNetOCR(nn.Module):
    def __init__(self, n_class, **kwargs):
        ...
        num_classes = 150
        ocr_mid_channels = OCR['MID_CHANNELS']
        ocr_key_channels = OCR['KEY_CHANNELS']
...
def hrnetocr(pretrained=False, **kwargs):
    model = HRNetOCR(n_class=1000, **kwargs)
    if pretrained:
        model.load_state_dict(load_url(model_urls['hrnetocr']), strict=False)
    return model
